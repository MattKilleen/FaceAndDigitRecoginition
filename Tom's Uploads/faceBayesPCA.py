...
#Function to calculate probability distribution function
def PDF(input):
   std = np.std(input)
   mean = np.mean(input)
   dist = norm(mean, std)
...
#Function to calculate probability given a sample, prior, and distributions
def calculateProb(x, prior, dists):
    prob = prior
...
print(str(PCA_DIM) + " Principal Components Capture " + str(round(100*sum/total, 2)) + "% of the Total Energy")
...
#The number of classes for faces
num_classes = 2
...
#Creating empty array of size 2 for each class of identification (face/not face)
...
#Empty PDF list
pdfs = [[None for j in range(PCA_DIM)] for i in range(2)]
...
for index, sample in enumerate(x_test):
    ...
    for idx in range(len(probabilities)):
        probabilities[idx] = calculateProb(sample,priors[idx],pdfs[idx])
...
# Print Time Taken
print("Time taken to test: {:.2f} seconds".format(test_time - end_time))